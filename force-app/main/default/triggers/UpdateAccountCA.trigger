trigger UpdateAccountCA on Order (after update) {
    ...
    AggregateResult[] groupedResults = [SELECT AccountId,SUM(TotalAmount)amt FROM Order where AccountId in:ids and Status='Ordered' group by AccountId]; //Query all order with status "Ordered" and group them by account
